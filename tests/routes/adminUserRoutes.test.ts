import {
  ...
  AuthTestServer,
} from "@tests/helpers/expressAuthTestHelper";
...
describe("Admin User Routes", () => {
  describe("GET /api/admin/user/users", () => {
    ...
    it("should return 401 without admin auth", async () => {
      const unauth = createAuthTestServer(
        adminUserRoutes,
        false,
        "/api/admin/user",
        "admin"
    ...
    it("should handle server errors", async () => {
      (userRepository.getAllUsers as jest.Mock).mockRejectedValueOnce(
        new Error("fail")
      );
      const res = await server.request.get("/api/admin/user/users");
  ...
  describe("GET /api/admin/user/users/:id/toggle", () => {
    it("should toggle user status", async () => {
      (userService.toggleUserStatus as jest.Mock).mockResolvedValueOnce({
        id: 1,
        status: true,
      });
    ...
    it("should handle server errors", async () => {
      (userService.toggleUserStatus as jest.Mock).mockRejectedValueOnce(
        new Error("fail")
      );
      const res = await server.request.get("/api/admin/user/users/1/toggle");
      expect(res.status).toBe(500);
  ...
  describe("GET /api/admin/user/users/:id/delete", () => {
    it("should delete user", async () => {
      (userService.softDeleteUser as jest.Mock).mockResolvedValueOnce({
        id: 1,
      });
    ...
    it("should handle server errors", async () => {
      (userService.softDeleteUser as jest.Mock).mockRejectedValueOnce(
        new Error("fail")
      );
      const res = await server.request.get("/api/admin/user/users/1/delete");
      expect(res.status).toBe(500);
